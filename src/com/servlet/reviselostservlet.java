...
public class reviselostservlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if((lostername==null ||lostername.length()<=0)||(lostname==null ||lostname.length()<=0)||(description==null ||description.length()<=0)||(address==null ||address.length()<=0)||(tel==null ||tel.length()<=0)){
			response.sendRedirect("page/foreground/lostandfind/reviselost.jsp");
		}
		else if(useDao.UpdateData(id,database,"lostername", "lostname","description","address","tel",lostername, lostname, description, address, tel)){

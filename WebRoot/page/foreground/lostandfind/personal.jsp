...
%>
<% String Promulgator=(String)session.getAttribute("username");%>
 ...
 <div id="contents">               
     ...
     <center>  <input type = "submit" style="width:200px;height:40px;"  value ="个人发布记录" name = "personal"/></center>
         ...
         <br>
           <br>
          <fieldset >
<legend align="center" ><h2>失物发布记录</h2></legend>

        <table align="center" border="1">
           <tbody>
                <tr><td width=25>编号</td><td width=75>失者姓名</td>  <td width=75>失物名称</td>   <td width=200>物品描述</td>    <td width=100>失者地址</td> <td width=100>失者电话</td>  <td width=50>发布者</td></tr> 

     <%
              request.setCharacterEncoding("utf-8");
              response.setCharacterEncoding("utf-8");
              
	       Vector V=(Vector)useDao.personalData("lost_message",Promulgator);//vector
	      ...
	      <label ><%=v.get(0).toString() %></label></td>
            ...
            <td><label ><%=v.get(6).toString() %></label></td>
             <td><a  href="revisedataserlet?lostre_id=<%=v.get(0).toString()%>" title="修改" >修改</a></td>
             <td><a  href="deletedataserlet?lost_id=<%=v.get(0).toString()%>" title="删除" >删除</a></td>
             <%
              }
...
<legend align="center" ><h2>拾到物发布记录</h2></legend>
<table align="center" border="2">

        
           <tbody>
                <tr><td width=25>编号</td><td width=75>拾到物名称</td>  <td width=75>拾到者姓名</td>   <td width=200>物品描述</td>    <td width=100>拾到者地址</td> <td width=100>拾到者电话</td>  <td width=50>发布者</td></tr> 
     ...
     <%
	       ...
	       Vector VV=(Vector)useDao.personalData("find_message",Promulgator);//vector
	       Iterator it1=VV.iterator();

	       while(it1.hasNext()){

	          Vector vv=(Vector)it1.next();

	           %>
	           <tr>
	           
	      <td>
	      <label ><%=vv.get(0).toString() %></label></td>
            <td><label ><%=vv.get(1).toString() %></label></td>
            <td><label ><%=vv.get(2).toString() %></label></td>
            <td><label ><%=vv.get(3).toString() %></label></td>
            <td><label ><%=vv.get(4).toString() %></label></td>
            <td><label ><%=vv.get(5).toString() %></label></td>
            <td><label ><%=vv.get(6).toString() %></label></td>
             <td><a  href="revisedataserlet?findre_id=<%=vv.get(0).toString()%>" title="修改" >修改</a></td>
             <td><a  href="deletedataserlet?find_id=<%=vv.get(0).toString()%>" title="删除" >删除</a></td>
             <%
              }
